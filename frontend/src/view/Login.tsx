...
const Login = () => {
  ...
  const handleCreateAccount = (event: React.MouseEvent<HTMLAnchorElement, MouseEvent>) => {
    event.preventDefault();
  ...
  const handleCreateAccountSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setError("Error al procesar la solicitud. Verifique mail incorrectamente escrito, mail institucional. Contraseña inválida.");
  };
      ...
      <div className="max-w-md w-full border-black bg-gradient-to-tl from-indigo-900 via-transparent hover:via-black to-red-900 sm:px-10 border rounded-xl sm:p-8 p-6">
        {!showCreateAccountForm && !showForgotPasswordForm ? (
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <button className="w-full border border-black bg-red-600 text-white py-2 rounded-md hover:bg-red-500 focus:outline-none focus:bg-red-900" type="submit">
              Iniciar sesión
            </button>
            <a href="#" onClick={handleForgotPassword} className="block mt-2" style={{ textDecoration: "none" }}>
              Olvidé mi contraseña
            </a>
            <a href="#" onClick={handleCreateAccount} style={{ textDecoration: "none" }} className="block mt-2">
              Crear cuenta
            </a>
          </form>
          ...
          <div className="flex justify-center items-center mt-8 mb-20">
            <div className="max-w-md w-full bg-gradient-to-tl from-indigo-900 via-transparent to-red-900 border border-black rounded-xl p-8">
              <form className="space-y-4" onSubmit={handleCreateAccountSubmit}>
                ...
                <button onClick={handleBackToLogin} className="w-full border border-black bg-red-600 text-white py-2 rounded-md hover:bg-red-500 focus:outline-none focus:bg-red-900">
          ...
          <div className="flex justify-center items-center mt-8 mb-20">
            <div className="max-w-md w-full bg-gradient-to-tl from-indigo-900 via-transparent to-red-900 border border-black rounded-xl p-8">
              <form className="space-y-4" onSubmit={handleForgotPasswordSubmit}>
                ...
                <button className="w-full bg-red-600 text-white py-2 rounded-md hover:bg-red-500 focus:outline-none focus:bg-red-900" type="submit">
                  Enviar
                </button>
                <button onClick={handleBackToLogin} className="w-full border border-black bg-red-600 text-white py-2 rounded-md hover:bg-red-500 focus:outline-none focus:bg-red-900">
                  Volver a inicio de sesión
